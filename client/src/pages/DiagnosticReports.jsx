...
export default function DiagnosticReports() {
  ...
  const filterReports = () => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(report => {
        const pet = pets.find(p => (p._id || p.id) === report.pet_id);
        const client = clients.find(c => (c._id || c.id) === report.client_id);
  ...
  const handleSubmit = async (reportData) => {
    try {
      if (editingReport) {
        await TenantDiagnosticReport.update(editingReport._id || editingReport.id, reportData);
      } else {
