...
export async function getFeedbackHandler(req, res) {
  try {
    ...
    const [pet, staff] = await Promise.all([
      appointment.pet_id ? db.collection('pets').findOne({ _id: appointment.pet_id }) : null,
      appointment.staff_id ? db.collection('staff').findOne({ _id: appointment.staff_id }) : null
    ]);
...
export async function submitFeedbackHandler(req, res) {
  try {
    ...
    const [pet, staff, client] = await Promise.all([
      updatedAppointment.pet_id ? db.collection('pets').findOne({ _id: updatedAppointment.pet_id }) : null,
      updatedAppointment.staff_id ? db.collection('staff').findOne({ _id: updatedAppointment.staff_id }) : null,
      updatedAppointment.client_id ? db.collection('clients').findOne({ _id: updatedAppointment.client_id }) : null
    ]);
