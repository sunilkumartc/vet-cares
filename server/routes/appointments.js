    ...
    appointments.map(async (appointment) => {
      ...
      if (appointment.client_id) {
        const owner = await db.collection("clients").findOne({
          _id: new ObjectId(appointment.client_id),
        })
        if (owner) {
...
// GET /api/appointment - Get all appointments for authenticated user
router.get("/", authenticateToken, async (req, res) => {
  try {
    const { page = 1, limit = 10, status, pet_id, upcoming } = req.query
    ...
    const query = {
      client_id: req.user.userId, // Use string instead of ObjectId for client_id
    }
    ...
    if (pet_id) {
      query.pet_id = pet_id // Use string pet_id as stored in database
    }

    if (upcoming === "true") {
      query.appointment_date = { $gte: new Date().toISOString().split("T")[0] } // Use appointment_date field
    ...
    let appointments = await appointmentsCollection
      ...
      .sort({ appointment_date: upcoming === "true" ? 1 : -1, appointment_time: 1 }) // Sort by appointment_date and time
      .toArray()
...
// GET /api/appointment/pet/:petId - Get appointments for specific pet
router.get("/pet/:petId", authenticateToken, async (req, res) => {
  try {
    ...
    const query = {
      pet_id: petId, // Use string pet_id as stored in database
      client_id: req.user.userId, // Use string client_id
    }
    ...
    let appointments = await appointmentsCollection
      ...
      .skip(skip)
      .limit(Number.parseInt(limit))
      .sort({ appointment_date: -1, appointment_time: -1 }) // Sort by appointment_date and time
      .toArray()
...
router.post("/", authenticateToken, async (req, res) => {
  try {
    const { petId, doctorId, clinicId, type, scheduledAt, duration = 30, reason, notes } = req.body
...
// PUT /api/appointment/:id/complete - Complete appointment
...
router.get("/stats", authenticateToken, async (req, res) => {
  try {
    ...
    const stats = await appointmentsCollection
      .aggregate([
        {
          $match: {
            client_id: new ObjectId(req.user.userId),
