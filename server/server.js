...
const corsOptions = {
  ...
  allowedHeaders: ['Content-Type', 'Authorization', 'Host', 'X-Requested-With','user-id', 'Accept'  ]
};
...
app.get('/api/test', async (req, res) => {
  try {
    ...
    res.json({ message: 'Server is working', tenantCount: count });
  } catch (error) {
...
// MongoDB connection - using new utilities
...
app.get('/api/staff', async (req, res) => {
  try {
    ...
    if (staff.length === 0) {
      // Try with ObjectId
      try {
        const objectId = await dbUtils.toObjectId(tenant_id);
        if (objectId) {
          staff = await collection.find({ tenant_id: objectId }).toArray();
    ...
    res.json(dbUtils.formatResponse(staff));
  } catch (error) {
...
app.post('/api/:entity', async (req, res) => {
  try {
    ...
    res.json(dbUtils.formatResponse(document));
  } catch (error) {
...
app.post('/api/staff/login', async (req, res) => {
  try {
    ...
    const tenant = await tenantCollection.findOne({ _id: await dbUtils.toObjectId(staff.tenant_id) });
...
app.get('/api/staff/verify', async (req, res) => {
  try {
    ...
    try {
      ...
      const collection = dbUtils.getCollection('staff');
      const staff = await collection.findOne({ _id: await dbUtils.toObjectId(staffId) });
...
app.get('/api/admin/verify', async (req, res) => {
  try {
    ...
    try {
      ...
      const collection = dbUtils.getCollection('system_admins');
      const admin = await collection.findOne({ _id: await dbUtils.toObjectId(adminId) });
    ...
    if (tenant_id) {
      try {
        const tenant = await dbUtils.getCollection("tenants").findOne({ 
          _id: await dbUtils.toObjectId(tenant_id) 
        });
        if (tenant) {
          tenantDetails = {
...
Thank you for choosing ${tenantDetails.clinicName}! 🐾`;
    ...
    await dbUtils.getCollection('whatsapp_messages').insertOne(messageRecord);
