...
export default function Layout({ children, currentPageName }) {
  ...
  const handleClientSelect = (client) => {
    ...
    setSearchTerm("");
    navigate(`${createPageUrl("ClientManagement")}?clientId=${client.id}`);
  };
            ...
            <ul className="space-y-2">
              {filteredClients.map((client) => {
                ...
                const lastVisit = medicalRecords
                  .filter((r) => clientPets.some((p) => p.id === r.pet_id))
                  .sort((a, b) => new Date(b.visit_date) - new Date(a.visit_date))[0];
                return (
                  <li key={client.id} className="p-3 rounded-lg hover:bg-blue-50 cursor-pointer transition-colors flex items-center justify-between" onClick={() => handleClientSelect(client)}>
                    <div>
