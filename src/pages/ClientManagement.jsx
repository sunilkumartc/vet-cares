...
import { TenantClient, TenantPet, TenantMedicalRecord, TenantVaccination, TenantInvoice, TenantMemo, TenantProduct, TenantProductBatch, TenantStockMovement, TenantStaff } from "@/api/tenantEntities";
...
export default function ClientManagement() {
  ...
  useEffect(() => {
    if (isFullScreen && selectedClient) {
      const allClientRecords = getClientRecords(selectedClient.id);
      const allClientVaccinations = getClientVaccinations(selectedClient.id);
      const allClientInvoices = getClientInvoices(selectedClient.id);
      const allClientMemos = getClientMemos(selectedClient.id);
      const allCombinedHistory = getCombinedHistory(selectedClient.id);
  ...
  const loadSelectedClientSpecificData = async (clientId) => {
    ...
    try {
      const [invoiceData, memoData, productData] = await Promise.all([
        TenantInvoice.filter({ client_id: clientId }, '-invoice_date'),
        TenantMemo.filter({ client_id: clientId }, '-created_date'),
        TenantProduct.list(), // Load all products for selection in forms
      ]);
  ...
  const handleFormSubmit = async (formData, formType) => {
    try {
      switch (formType) {
        case 'client':
          if (editingRecord) {
                    await TenantClient.update(editingRecord.id, formData);
      } else {
        await TenantClient.create(formData);
          }
          break;
        case 'pet':
          if (editingRecord) {
                    await TenantPet.update(editingRecord.id, formData);
      } else {
        await TenantPet.create({ ...formData, client_id: selectedClient.id });
          }
          break;
        case 'medical':
          if (editingRecord && editingRecord.id) { // Check for existing ID to update
            await TenantMedicalRecord.update(editingRecord.id, formData);
          } else { // Create new, potentially with pre-filled pet_id
            await TenantMedicalRecord.create(formData);
          }
          break;
        case 'vaccination':
          if (editingRecord && editingRecord.id) { // Check for existing ID to update
            await TenantVaccination.update(editingRecord.id, formData);
          } else { // Create new, potentially with pre-filled pet_id
            await TenantVaccination.create(formData);
          }
          break;
        case 'memo': // TenantMemo is still handled by showForm
          if (editingRecord) {
            await TenantMemo.update(editingRecord.id, formData);
          } else {
            await TenantMemo.create(formData);
          }
          break;
      }
  ...
  const getClientRecords = (clientId) => {
    ...
    return medicalRecords.filter(r => petIds.includes(r.pet_id));
  };
  const getClientVaccinations = (clientId) => {
    const clientPets = getClientPets(clientId);
    const petIds = clientPets.map(p => p.id);
    return vaccinations.filter(v => petIds.includes(v.pet_id));
  };
  ...
  const getClientMemos = (clientId) => {
    ...
    return clientMemos.filter(m =>
      (m.client_id === clientId || (m.pet_id && petIds.includes(m.pet_id))) &&
      m.is_active !== false
    ); // Only active memos
  };
  ...
  if (showForm) {
    ...
    const forms = {
      ...
      medical: <MedicalRecordForm record={editingRecord} pets={getClientPets(selectedClient?.id)} clients={selectedClient ? [selectedClient] : clients} veterinarians={veterinarians} onSubmit={(data) => handleFormSubmit(data, 'medical')} onCancel={handleCancel} />,
      vaccination: <VaccinationForm vaccination={editingRecord} pets={getClientPets(selectedClient?.id)} clients={selectedClient ? [selectedClient] : clients} veterinarians={veterinarians} onSubmit={(data) => handleFormSubmit(data, 'vaccination')} onCancel={handleCancel} />,
    ...
    return (
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
        <InvoiceForm
          ...
          pets={getClientPets(selectedClient?.id)}
  ...
  if (isFullScreen && selectedClient) {
    const clientPets = getClientPets(selectedClient.id);
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="bg-gray-100 px-6 py-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {clientPets.map(pet => {
                const petRecords = getClientRecords(selectedClient.id).filter(r => r.pet_id === pet.id);
                const recentRecord = petRecords.sort((a, b) => new Date(b.visit_date) - new Date(a.visit_date))[0];
                const petVaccinations = getClientVaccinations(selectedClient.id).filter(v => v.pet_id === pet.id);
                ...
                return (
                  <Card 
                    ...
                    className={`cursor-pointer hover:shadow-lg transition-all duration-300 border-2 ${
                        selectedPetId === pet.id ? 'border-blue-500 shadow-xl bg-blue-50' : 'hover:border-blue-300 bg-white'
                    }`}
                  >
